  ...
  return (
    <div className="bg-white/95 backdrop-blur-xl p-6 sm:p-8 rounded-[2.5rem] shadow-2xl border border-white flex flex-col items-center text-center max-w-lg w-full overflow-y-auto max-h-full no-scrollbar z-50">
      <div className="flex items-center gap-3 mb-2">
        <h1 className="text-3xl sm:text-4xl font-black text-slate-800 tracking-tight">连连看</h1>
        <div className="bg-indigo-100 text-indigo-600 px-3 py-1 rounded-full text-[10px] font-black flex items-center gap-1">
          <Activity size={10} />
      ...
      <div className="w-full space-y-6">
        <section className="text-left">
          <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 flex items-center gap-2">
            <span className="w-4 h-4 rounded-full bg-indigo-500 text-[8px] flex items-center justify-center text-white">1</span>
            选择主题
          </h3>
          <div className="grid grid-cols-3 gap-2">
            {THEMES.map(t => (
              ...
              >
                ...
                <span className="text-[8px] font-bold text-slate-600 truncate w-full text-center">{t.name}</span>
              </button>
            ))}
          </div>
        </section>
        ...
        <section className="text-left pb-4">
          <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 flex items-center gap-2">
             <span className="w-4 h-4 rounded-full bg-indigo-500 text-[8px] flex items-center justify-center text-white">2</span>
             选择关卡
          </h3>
              ...
              return (
                ...
                >
                  <div className="flex justify-between items-start">
                    <div className="font-black text-slate-800 text-xs group-hover:text-indigo-600">{l.name}</div>
                    {best !== undefined && <Medal size={14} className="text-yellow-500" />}
                  </div>
                  <div className="text-[8px] text-slate-400 font-bold uppercase tracking-tighter">{l.cols}x{l.rows} • {l.difficulty}</div>
                  {best !== undefined && (
                    <div className="mt-1 text-[8px] font-black text-indigo-500 bg-indigo-50 inline-block px-1.5 py-0.5 rounded">
                      记录: {formatTime(best)}
                    </div>
                  )}
                </button>
